...
router.get('/:restaurant_id', async (req, res) => {
  try {
    ...
    promos.forEach(p => {
      if (p.plat_id) { const plat = plats.find(pl => pl._id === p.plat_id); if (plat) plat.promo_prix = p.type === 'pourcentage' ? Math.round(plat.prix * (1 - p.valeur/100)) : plat.prix - p.valeur; }
    });
    const categories = {};
    plats.forEach(p => { if (!categories[p.categorie]) categories[p.categorie] = []; categories[p.categorie].push(p); });
    const avis = (await db.avis.findAsync({ restaurant_id: req.params.restaurant_id })).sort((a,b) => new Date(b.createdAt)-new Date(a.createdAt)).slice(0,6);
    res.render('client/menu', { restaurant, categories, plats, avis });
