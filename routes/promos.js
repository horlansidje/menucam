...
router.post('/nouvelle', requireAuth, async (req, res) => {
  ...
  await db.promos.insertAsync({
    restaurant_id: req.session.restaurantId,
    code: code.toUpperCase(), type, valeur: parseFloat(valeur),
    min_commande: parseFloat(min_commande)||0,
    max_utilisations: parseInt(max_utilisations)||999,
    utilisations: 0,
    date_fin: date_fin ? new Date(date_fin) : null,
    description: description||'', actif: true,
    createdAt: new Date()
  });
  req.flash('success', `Code promo ${code.toUpperCase()} créé !`);
  res.redirect('/promos');
});

router.post('/:id/toggle', requireAuth, async (req, res) => {
  const promo = await db.promos.findOneAsync({ _id: req.params.id, restaurant_id: req.session.restaurantId });
  if (promo) await db.promos.updateAsync({ _id: promo._id }, { $set: { actif: !promo.actif } });
  res.json({ ok: true, actif: !promo?.actif });
});
...
// API: vérifier un code promo (depuis menu client)
router.post('/verifier', async (req, res) => {
  ...
  if (!promo) return res.json({ ok: false, message: 'Code invalide ou inactif.' });
  ...
  if (total < promo.min_commande) return res.json({ ok: false, message: `Commande minimum : ${promo.min_commande.toLocaleString('fr-FR')} FCFA` });
